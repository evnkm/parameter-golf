"""
SOTA parameter-golf training script.

Architecture: 10-layer GPT, dim=512, 8 heads (4 KV), 3× MLP (ReLU²),
SmearGate, BigramHash(10240), U-Net skips, tied FP16 embeddings.
Quantization: mixed int5 (MLP) / int6 (attention) post-training + zstd-22.
Training: Muon (WD=0.04) + AdamW, SWA over last 40% of warmdown, magnitude pruning.
Evaluation: sliding window at stride=64, seq_len=2048.
"""
...
class Hyperparameters:
    ...
    qat_clip_mlp = int(os.environ.get("QAT_CLIP_MLP", 15))
    qat_clip_attn = int(os.environ.get("QAT_CLIP_ATTN", 31))
...
# ---------------------------------------------------------------------------
# Post-training quantization: mixed int5 (MLP) / int6 (attention)
# ---------------------------------------------------------------------------
...
def fake_quantize_ste(t: Tensor, clip_range: int = 31) -> Tensor:
    """STE fake quantization: quantize in forward, pass gradients through."""
    if not t.is_floating_point() or t.numel() <= 8192 or t.ndim < 2:
        return t
    t32 = t.float()
    row_max = t32.detach().abs().amax(dim=-1, keepdim=True)
    scale = (row_max / clip_range).clamp_min(1e-12)
    q = torch.clamp(torch.round(t32 / scale), -(clip_range + 1), clip_range)
    return (t + (q * scale - t32).detach()).to(t.dtype)
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.qat_clip > 0:
            w = fake_quantize_ste(w, self.qat_clip)
        return F.linear(x, w,
...
class TrigramHashEmbedding(nn.Module):
    ...
    def trigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        out[..., 2:] = torch.bitwise_xor(
            torch.bitwise_xor(51929 * t[..., 2:], 36313 * t[..., 1:-1]),
            27191 * t[..., :-2]
        ) % mod
        return out.long()
...
class GPT(nn.Module):
    ...
    def _logits(self, x: Tensor) -> Tensor:
        if self.tie_embeddings:
            raw = F.linear(x, self.tok_emb.weight.to(x.dtype))
        else:
            raw = self.lm_head(x)
    ...
    def enable_qat(self, clip_mlp: int = 15, clip_attn: int = 31) -> None:
        """Set STE fake-quantization clip ranges on all CastedLinear layers."""
        for name, module in self.named_modules():
            if isinstance(module, CastedLinear) and module.weight.numel() > 8192:
                if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
                    continue
                if any(p in name for p in FP16_KEEP_NAME_PATTERNS):
                    continue
                module.qat_clip = clip_mlp if ".mlp." in name else clip_attn
...
def main() -> None:
    ...
    if args.qat_enabled:
        base_model.enable_qat(args.qat_clip_mlp, args.qat_clip_attn)
        log0(f"qat:enabled from start clip_mlp={args.qat_clip_mlp} clip_attn={args.qat_clip_attn}")
    ...
    log0(f"params:{n_params} layers:{args.num_layers} dim:{args.model_dim} heads:{args.num_heads} kv:{args.num_kv_heads}")
    log0(f"world:{world_size} accum:{grad_accum_steps} batch:{args.train_batch_tokens} seq:{args.train_seq_len}")
    ...
    if args.swa_enabled and swa_state is not None and swa_count > 1:
        log0(f"swa:applying {swa_count} checkpoints")
        current = base_model.state_dict()
        avg = {n: (t / swa_count).to(dtype=current[n].dtype) for n, t in swa_state.items()}
    ...
    if master_process:
        with open("final_model.int8.ptz", "wb") as f:
            f.write(quant_blob)
        qsize = os.path.getsize("final_model.int8.ptz")
        csize = len(code.encode("utf-8"))
        log0(f"compressed:{qsize} code:{csize} total:{qsize + csize} compressor:{_COMPRESSOR}")
    ...
    quant_loaded = torch.load(io.BytesIO(raw), map_location="cpu")
    deq = dequantize_mixed(quant_loaded["w"], quant_loaded["m"], sd_cpu)
    ...
    if args.eval_stride > 0 and args.eval_stride < args.train_seq_len:
        ...
        q_loss, q_bpb = eval_val(
            ...
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut)
    torch.cuda.synchronize()
    log0(f"final val_loss:{q_loss:.4f} val_bpb:{q_bpb:.4f} eval_time:{1000 * (time.perf_counter() - t_eval):.0f}ms")
